...
function FileUpload({ onUploadStart, onUploadComplete, loading }) {
  ...
  const handleDownloadTemplate = async (templateType = 'minimum') => {
    try {
      const response = await axios.get(`${API_BASE_URL}/bookings/template/${templateType}`, {
        responseType: 'blob'
      });
      
      // Determine filename based on template type
      const filename = templateType === 'minimum' 
      ...
      setError('Failed to download template. Please check if the backend server is running.');
  ...
  return (
    <div className="file-upload-container card">
      ...
      <div className="template-download">
        ...
        <p className="template-description">Choose the template that best fits your needs:</p>
        <div className="template-buttons">
          <div className="template-option">
            <button 
              className="btn btn-secondary" 
              onClick={() => handleDownloadTemplate('minimum')}
              disabled={loading}
            >
              📄 Minimum Template
            </button>
            <span className="template-info">18 essential fields - Quick & simple</span>
          </div>
          <div className="template-option">
            <button 
              className="btn btn-secondary" 
            ...
            <span className="template-info">47 comprehensive fields - Full details</span>
          </div>
        </div>
      </div>
