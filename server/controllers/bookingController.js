...
async function downloadTemplate(req, res) {
  try {
    const templatePath = path.join(__dirname, '../../public/Resource_Booking_Template.xlsx');
    
    // Check if template exists
    try {
      await fs.access(templatePath);
      return res.download(templatePath, 'Resource_Booking_Template.xlsx');
    } catch (error) {
      return res.status(404).json({
        success: false,
        message: 'Template file not found. Path checked: ' + templatePath
      });
    }
  } catch (error) {
    return res.status(500).json({
